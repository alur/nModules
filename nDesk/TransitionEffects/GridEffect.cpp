...
void GridEffect::Start(ID2D1BitmapBrush* oldBrush, ID2D1BitmapBrush* newBrush) {
	...
	// Generate values in the range [0,0.8]
	int i = 0, x = 0, y = 0;
	switch (m_gridType) {
	case RANDOM:
		for (i = 0; i < m_iSquares; i++) {
			m_StartTimes[i] = (rand() % 8000) / 10000.0f;
		}
		break;
	...
	case LINEAR_HORIZONTAL:
		for (x = 0; x < m_iSquaresX; x++) {
			for (y = 0; y < m_iSquaresY; y++) {
				m_StartTimes[y*m_iSquaresX + x] = (i++)*0.8f/m_iSquares;
			}
		}
		break;
	...
	case LINEAR_VERTICAL:
		for (i = 0; i < m_iSquares; i++) {
			m_StartTimes[i] = i*0.8f/m_iSquares;
		}
		break;
	}
}
...
void GridEffect::Paint(ID2D1RenderTarget* renderTarget, float fProgress) {
	...
	for (int i = 0; i < m_iSquares; i++) {
		m_pNewBrush->SetOpacity(min(1.0f, max(fProgress - m_StartTimes[i], 0.0f)*5));
